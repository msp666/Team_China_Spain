...
def openDoor(C: ry.Config):
    ...
    M.setup_sequence(C, 3, 1e-2, 1e-1, True, False, False)
    ...
    M.freeze_joint([0, 2.], ['door_joint'])
    M.komo.addObjective([2], ry.FS.qItself, ['handle_joint'], ry.OT.eq, [1e1], [0.5])
    M.komo.addObjective([3], ry.FS.qItself, ['door_joint'], ry.OT.eq, [1e1], [-0.8])
    ...
    M.komo.addObjective([1, 3], ry.FS.poseDiff, ['handle_body2', 'grasp_handle'], ry.OT.eq, [1e1], [0.])
    ...
    '''THe M1.no_collisions function is quite important, it can garantee that the gripper does not collide with obj before approaching'''
    # M1.no_collisions([0.2, 0.9], col_pairs, margin=0.05)
    ...
    M2 = M.sub_motion(1)
    M2.freeze_joint([0., 1.], ['door_joint'])
    # M2.no_collisions([0., 1.], col_pairs, margin=0.001)
    ...
    M3 = M.sub_motion(2)
    M3.freeze_relativePose([0., 1.], gripper, 'handle_body2')
    # M3.no_collisions([0., 1.], col_pairs, margin=0.001)
    M3.solve()
    if not M3.feasible:
        print(M3.ret)
        return
    ...
    path3 = M3.path[:, 0:ROBOT_JOINT_INDEX]
